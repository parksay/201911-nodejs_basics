    ...
    if(pathname === '/'){
        if(queryData.id === undefined){
            ...
            fs.readdir('./data',function(error, filelist){
                ...
                fs.readFile(`data/${filteredId}`,"utf8",function(err, file_content){
                    var sanitizedFile_Content = sanitizeHtml(file_content);
                    //even though there is a sentence "<script>alert("merong");</script>" on the file,
                    //the danger, dirty contents are erazed when it is send on the page
                    response.writeHead(200);
                    response.end(html + sanitizedFile_Content);
                });
      ...
      fs.readdir('./data',function(error, filelist){
        ...
        //if you recieve the content as data from client like "<script>location.href='https://opentutorials.org/course/1';</script>"
        //it becomes an file. and it would occur serius problem.
        //so, let's download "sanitize-html" open cmd window and access your package's directory
        //npm init
        //press enter key sevral time
        //npm install -S sanitize-html
        //then you can find a package.json file and node_modules folder
        //even though there is a sentence "<script>alert("merong");</script>" on the file,
        //the danger, dirty contents are disappeared when it is send on the page
